...
description: "Tour the MessageDesk shared inbox—how it groups SMS threads, surfaces team activity, and keeps everyone aligned with assignments, labels, and filters."
...
- **Thread:** Texts + internal **comments** (clearly styled) + **emoji reactions**
- **Header actions (top-right):** **Assign**, **Labels**, **Open/Close**, **Create Contact**
- **More Options ⋯ menu:** Opens conversation-level actions including **[Export](/inbox/export-conversation)** to CSV or PDF
...
<Note>
  **Deleting messages from a thread is restricted to Owners.** Only users with **Owner** permission on a phone number's Inbox Access can delete individual messages from a conversation thread. By default, only **Admins** have Owner access to all phone lines. To prevent a user from deleting messages, make sure their role does **not** have Owner permission on that phone line. See [Team Management → Permissions](/settings/workspace-settings/team-management#what-each-permission-level-grants-access-to) for details.
</Note>
