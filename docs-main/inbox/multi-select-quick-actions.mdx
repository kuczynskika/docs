...
description: "Bulk-manage SMS threads in MessageDesk. Multi-select conversations to close, assign, label, archive, or delete dozens of messages in a single click."
sidebarTitle: "Multi-select quick actions"
"og:title": "Multi-Select Quick Actions - Bulk Conversation Management"
"og:description": "Select multiple conversations at once and apply bulk actions."
"og:image": "https://uploads-ssl.webflow.com/6349fd438ecb3a3594605225/63dd3a9bd4c06c39183e7b60_messagedesk-og-1200x630.webp"
---

# Multi-Select Quick Actions from Any Inbox View

Select multiple conversations and apply bulk actions straight from the conversation list. Works in **sorted**, **filtered**, and **pre-saved** inbox views.

---

## Multi-select and apply quick actions
...
- **Apply label.** Add a label to every selected conversation.
- **Assign to teammate.** Bulk-assign every selected conversation to one teammate.
- **Clear selection.** Unselect everything.

![Inbox Select Pn](/images/inbox-select.png)

<Tip>
  Need to message **more than 25** people at once? Use **Contacts** → multi-select → **Compose** to send a broadcast to **up to 100** recipients (send individually).
</Tip>
...
Yes. Use **Apply label** and select every label you want to add.

**Will bulk assign overwrite current owners?**\
Yes. The selected teammate becomes the assignee on **every** selected conversation.
...
- **Actions are greyed out.**\
  Make sure at least one conversation is selected and that you have permission to perform that action.
- **Group Message option missing/greyed out.**\
  You probably selected more than **7** conversations. Deselect until you're at 7 or fewer.
- **Reply-All is greyed out.**

  Make sure you don't have a group conversation selected in your list. Reply-All doesn't work on group threads.
