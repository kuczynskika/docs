...
<Note>
  **Opt-outs are enforced automatically.** If the recipient has opted out in MessageDesk, the **Send Message** action will fail and no text will be delivered — even if the Zap runs successfully on Zapier's side. Opt-out enforcement applies to **both V1 and V2** of the Zapier integration and can't be bypassed. See [Compliance Center](/settings/workspace-settings/compliance-center) for how opt-outs are captured and managed.
</Note>
...
| **Send Message action fails for a specific contact**    | Confirm the contact hasn't opted out. MessageDesk automatically blocks outbound messages to opted-out contacts, so the Zap action will fail even though the Zap itself runs. See [Compliance Center](/settings/workspace-settings/compliance-center). |
