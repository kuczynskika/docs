...
<Tip>
  **Stacking templates on the same Trigger?** When you build more than one Relay on the same trigger (for example, an after-hours reply and a keyword reply both on **Message Received**), set the **Order** on the Relays list so the more specific Relay runs first. All matching Relays run in Order. See [Order your Relays](/relays/create-edit#1a-order-your-relays).
</Tip>
...
<Note>
  **Confirmed vs. pending.** The Add Action menu can vary by trigger. The templates below are limited to combinations that are confirmed to appear in the app today. Three additional foundational templates (on **Conversation Opened**, **Comment Created**, and **Conversation Label Applied**) are staged for a follow-up once their action menus are confirmed.
</Note>
...
The templates below are grouped by the way MessageDesk teams tend to use them. The **Core Four** sections are the primary focus. **Also served (SMB)** and **Support and sales (cross-vertical)** cover common patterns that show up across every vertical.
...
### Core Four: construction and dispatch
...
**Why it works:** Inbound texts land in a shared inbox with an owner and a job label, so a thread stays covered when a supervisor is on a site or out.
...
**Why it works:** Anything that slips past routing gets flagged instead of aging silently.

---

### Core Four: logistics and trucking
...
**Why it works:** Every dispatcher sees every message in one inbox and texts self-sort by region, replacing the single shared phone.
...
**Why it works:** An off-hours text gets an immediate acknowledgement and is waiting for dispatch in the morning.
...
**Why it works:** Pair a broadcast with a keyword reply and every acknowledgement is labeled and logged automatically.

---

### Core Four: oil and gas
...
**Why it works:** Every candidate availability reply is captured, owned, and pushed to your system, so nobody gets called twice.
...
### Core Four: medical staffing, recruiting, and HR
...
**Why it works:** After a shift-fill text, replies route to the coordinator so the fastest responder is placed first and the reply is logged.
...
### Enterprise professional services (legal intake)
...
These verticals are supported but are not the primary focus. The patterns below cover the most common SMB use cases.
...
**Why it works:** The lead is captured and routed even when the office is dark.

---

### Support and sales (cross-vertical)
...
<Tip>
  Keep a keyword-free catch-all Relay ordered **last** so any message that does not match a keyword still gets a graceful reply and an owner.
</Tip>
...
**Why this works:** MessageDesk's in-app notifications reach assignees inside the product, but teams that live in Slack or Teams want a ping in their chat tool the moment a conversation lands in their queue. Fire a webhook on **Conversation Users Assigned** and route the payload to your notifier of choice.
