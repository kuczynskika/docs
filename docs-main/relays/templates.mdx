...
<Note>
  **Call Received works on MD-owned numbers only.** It does not fire for Twilio-connected numbers, Telegorithm numbers, or hosted (text-enabled) numbers. **Reply to message** isn't available on this trigger. See [Call Received actions](/relays/building-blocks#actions-on-the-new-triggers) for the full action list.
</Note>
