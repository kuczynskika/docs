...
| **Auto-Close No-Reply-Needed** | Closes conversations labeled `No Reply Needed` and clears the label when a reply arrives | Message Received + Label name equals + Close the conversation + Remove Label where name equals |
...
| **After-Hours Auto-Reply (Keep Unread)** | Texts an autoresponse outside operating hours and keeps the thread Unread for morning review | Message Received + Active hours (inverted) + Reply to message + Mark Conversation Unread |
...
<Note>
  **Verify before enabling.** The **Message failure contains text** conditions with the v2026.07.15 release. If your workspace doesn't see it on the Message Failed trigger yet, hold this template until it appears.
</Note>
