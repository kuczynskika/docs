...
<Note>
  **Inbound Webhook Received is a non-conversation trigger.** Conversation-scoped conditions and actions don't apply. See [Webhooks](/relays/webhooks#inbound-webhooks) for the endpoint URL, the `X-MD-Signature` header, and the payload schema.
</Note>
