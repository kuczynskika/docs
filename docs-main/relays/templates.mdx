...
**Why it works:** The first teammate to reply gets assigned, and because the conversation is then no longer unassigned, later replies from others do not steal it. Follow-up never lands in an unclaimed pile. This is the outcome behind the shipped **Auto Assign to First Responder** starter.
...
**Why this works:** There is no built-in "contact is opted out" condition on the trigger. Build the handler from conditions that do exist: match the STOP keyword on the inbound message itself, or route on an `Opted Out` label that was applied upstream (by a keyword Relay, by a teammate, or by an integration). Labeling the thread keeps it visible to compliance and support, and routing it to your support queue ensures someone confirms the customer's request was honored. Once `Opted Out` is applied, downstream filters and Relays can avoid touching that contact.
