...
| Contact Updated               | A contact property changes on a saved contact                             | New in v2026.08.11. Non-conversation trigger: it fires against a contact, not a conversation, so conversation-scoped conditions and actions don't apply. Pairs with the **Send Message** action, which lets you pick the outbound channel to send from. |
...
- **Send Message** (new in v2026.08.11) sends an outbound SMS/MMS on non-conversation triggers. Because there's no conversation to reply into, you pick which connected phone number (**Outbound Channel**) the message sends from. Currently available on **Contact Updated**.
...
**Contact Updated** is a non-conversation trigger. It fires against a contact when a contact property changes, so there's no conversation in scope: conversation-scoped conditions (for example, **Is unassigned conversation**, **Teammate Assigned**, **Conversation label equals**) and conversation-scoped actions (assignment, labels, close/open, comments) don't apply. Supported actions:

- Send Message
- Send webhook

<Note>
  **Pick the Outbound Channel on Send Message.** Because there's no existing conversation to reply into, the **Send Message** action asks which connected phone number to send from. MessageDesk creates or continues a conversation with the contact on that channel, and standard compliance (opt-outs, 10DLC) applies.
</Note>
