...
**Inbound Webhook Received** is a non-conversation trigger. An outside system sends the payload, so there's no conversation in scope and conversation-scoped conditions and actions don't apply. It pairs well with:
