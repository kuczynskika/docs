...
| Call Received                 | An MD-owned phone number receives an inbound call                         | New in v2026.08.04. Only fires on MessageDesk-owned numbers. Does not fire for Twilio-connected, Telegorithm, or hosted (text-enabled) numbers. |
...
<Note>
  **Call Received works on MD-owned numbers only.** It does not fire for Twilio-connected numbers, Telegorithm numbers, or hosted (text-enabled) numbers where voice stays with the original provider. See [Connect phone numbers](/get-started/phone-numbers/overview) for the difference between owned, hosted, and Twilio-connected lines.
</Note>
