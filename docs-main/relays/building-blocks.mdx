...
- **Update Contact Property** updates a contact property you choose. Set it to a **Custom Value**, or pull a value from the triggering event (**Event Data**). A regular **Text** property can take either a Custom Value or an Event Data value. A non-text property (for example dropdown or number) can only take a static value that matches its type.
...
- On **Update Contact Property**, the same choice applies for **Text** properties.
