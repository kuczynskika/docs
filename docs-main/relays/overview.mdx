...
- **Triggers:** When it runs\
  _Examples:_ `MessageReceived` (a new inbound text starts the relay), `MessageSent` (an outbound message is sent from your workspace), `CommentCreated` (a teammate posts an internal comment), `ConversationOpened` (a closed conversation is reopened), `ConversationAssigned` (one or more teammates are assigned to a conversation), `ConversationClosed` (a teammate closes a conversation)
