...
When a Relay sends an outbound message (for example, an after-hours or out-of-office auto-reply), MessageDesk updates the conversation's status the same way it would for a message sent by a teammate.
...
<Note>
  **Auto-replies do not mark a conversation as Read.** A Relay reply moves the conversation out of the **Unread** filter and into **Replied**, but the conversation is only marked **Read** when a teammate actually opens it. This means after-hours threads still surface for human review the next morning, even though the customer already got an automated acknowledgement.
</Note>

This behavior is the same for every Relay action that sends a message (after-hours auto-reply, keyword auto-reply, anonymous-contact first-touch, etc.).
...
No. When a Relay sends an auto-reply, the conversation moves to **Replied** status (so it leaves the **Unread** filter), but it is **not** marked **Read**. A conversation is only marked Read when a teammate opens it in the Inbox. See [Relays and conversation status](#relays-and-conversation-status).
