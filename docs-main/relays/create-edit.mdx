...
- **Send Comment** (also called Post a Comment): posts an internal comment on the conversation. Supports static text and tags, is never sent to the contact, and is attributed as relay-generated. Use it for handoff notes and captured context.
- **Stop Processing:** a terminal control action. When a Relay fires and reaches this action, no lower-priority Relays on the same trigger run. Delayed or time-based Relays are exempt. Use it for clean routing, so a matched Relay can prevent a catch-all from also firing.
...
To scope an out-of-office to one teammate, add the **Teammate Assigned** condition set to that teammate. This works on a shared line: the auto-reply fires only for conversations assigned to the person who is out, while everyone else on the line keeps working. To also catch brand-new, unassigned threads, add **Is unassigned conversation** with **OR**. If the teammate has a dedicated line, you can scope by **My phone number equals** instead. See [Can I set up an out-of-office reply for just one person?](#can-i-set-up-an-out-of-office-reply-for-just-one-person) in the FAQ.
...
Yes, including on a shared line. Build the out-of-office Relay (Message Received, plus Active hours with **Trigger outside of business hours** enabled), then add the **Teammate Assigned** condition set to the teammate who is out. The auto-reply then fires only for that person's conversations, while the rest of the team keeps working on the same line. To also cover brand-new threads no one has picked up yet, add **Is unassigned conversation** with **OR**. If the teammate has their own dedicated line, you can instead scope by **My phone number equals** that line.
