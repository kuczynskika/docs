...
Relays are **grouped by Trigger** (Message Received, Message Sent, Comment Created, Conversation Closed) and, within each group, **sorted by Order** — so you can scan every automation on a phone line or event at a glance.
...
There is no drag-and-drop; the counter is the only way to reorder.
...
<Tip>
  Put **narrow, specific** Relays (keyword matches, anonymous-contact capture) **above** broad catch-alls (after-hours auto-reply) so the specific behavior runs first and downstream actions like **Add label** or **Assign** layer on top in the order you expect.
</Tip>
...
- **Relay order:** Within a Trigger group, Relays run top-down in the **Order** you set on the Relays list. All matching Relays run — a higher-priority Relay does not short-circuit later Relays. Reorder from the **Order** column using the up/down arrows or by typing the numeric value.
