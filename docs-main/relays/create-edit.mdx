...
* **Message Failed:** Fires when an outbound message fails to deliver. Pair with the **Message failure contains text** condition to react to specific carrier or provider errors (e.g., `landline`, `unreachable`, `blocked`). Useful for surfacing failures to a teammate, opening the conversation for human review, labeling for follow-up, or forwarding the failure event to an external system. See [Delivery status and errors](/messaging/delivery-status-errors).
...
* **Conversation Label Removed:** Fires when a label is removed from a conversation. Mirror of Conversation Label Applied. Useful for reversing label-driven state — for example, reopening a conversation when a `Snoozed` label is cleared, un-assigning on a `VIP` removal, or notifying an external system that a workflow tag was pulled off.
...
- **Conversation has label:** a boolean presence check for a specific label. Use this as a gate — run the Relay only when the conversation is already labeled (e.g., `VIP`, `Escalated`).
