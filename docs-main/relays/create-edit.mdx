...
Relays are **grouped by Trigger** (Message Received, Message Sent, Conversation Opened, Conversation Users Assigned, Conversation Label Applied, Comment Created, Conversation Closed) and, within each group, **sorted by Order**. You can scan every Relay on a phone line or event at a glance.
...
<Note>
  **Changing the trigger on an existing Relay.** If you switch a Relay's trigger, MessageDesk removes any conditions the new trigger cannot support. The app shows a warning with a count before you save (for example, "Changing the trigger will remove 1 incompatible condition from this relay"). Review your conditions after switching triggers.
</Note>
...
The available actions can vary by trigger, so check the **Add Action** menu in the app for the trigger you are using.
...
On a shared line, this pattern does not work. An out-of-office Relay on a shared line replies to every inbound message during its active-hours window, so it applies to the whole team, not to one person. When a teammate on a shared line is out, reassign their open conversations to someone else instead of using an auto-reply. See [Can I set up an out-of-office reply for just one person?](#can-i-set-up-an-out-of-office-reply-for-just-one-person) in the FAQ.

<Tip>
  When each teammate has their own line and their own out-of-office Relay, give each Relay a clear descriptive name so they are easy to manage:
