...
<Note>
  **Auto-replies do not mark a conversation as Read.** When this Relay sends its after-hours reply, the conversation moves to **Replied** status (so it leaves the Unread filter), but it is **not** marked as Read. A conversation is only marked Read when a teammate opens it in the Inbox. See [Relays and conversation status](/relays/create-edit#relays-and-conversation-status) for details.
</Note>
