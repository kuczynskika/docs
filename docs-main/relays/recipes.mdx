...
**Why this works:** When a shared inbox has no owner on a thread, the first teammate to reply usually should own the follow-up. This Relay watches outbound messages, checks that no one is assigned yet, and automatically assigns the conversation to whoever hit send. New inbound messages on the same thread will then route to that teammate by default.
