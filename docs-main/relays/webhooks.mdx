...
MessageDesk supports webhooks in both directions. Use the Send Webhook action to send event data out of MessageDesk to your endpoint when a Relay fires. Use the Inbound Webhook Received trigger to send data into MessageDesk from an outside system and start a Relay. This page covers both.
...
MessageDesk expects a fixed payload shape. There is no field mapping inside MessageDesk, so shape your request to match this schema before you send it, using an automation platform or your own code.
...
- Inbound webhooks are limited to 10,000 requests per day per workspace.
- Messaging compliance still applies to any message a webhook triggers. Opt-outs are honored, and automated messages should include opt-out language such as "Reply STOP to opt out."
