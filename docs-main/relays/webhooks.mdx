...
Payload values arrive as strings. MessageDesk checks each `properties` value against its Custom Property's type and accepts it only if it fits. See [Custom Fields](/settings/workspace-settings/custom-fields#supported-field-types) for the field types.
...
Say `Favorite Color` is a dropdown with the options `Red`, `Green`, and `Blue`. MessageDesk accepts only those three values, spelled exactly. A value of `red` or `Purple` is rejected.

### Auto-create contacts

Each action on a Relay that uses the **Inbound Webhook Received** trigger includes a contact **Auto-Create** checkbox. The checkbox is set per action, and it only appears when the Relay's trigger is **Inbound Webhook Received**.
