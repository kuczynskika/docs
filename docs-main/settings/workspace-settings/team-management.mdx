...
og:description: "Invite teammates, accept invitations, and manage access with roles and permissions in MessageDesk."
...
<Warning>
  Inviting teammates increases your **seat count** and updates your **subscription** automatically. Learn more: [Billing & Subscription](/settings/workspace-settings/billing-subscriptions).
</Warning>
...
- All features & resources (including account/billing): **Owner**.
...
| --- | --- | --- | --- | --- |
| **Phone number inboxes** | Full access and ownership for every conversation in this Inbox. **Only Owners can delete individual messages from a conversation thread.** Can import and export conversations in bulk. | Can create conversations, interact with, and view conversations for this Inbox. **Can't delete messages from a conversation thread.** No bulk import or export. | Can view and interact with conversations in this inbox, but can't create new conversations or delete them. **Can't delete messages from a conversation thread.** | Blocked from viewing, editing, or deleting conversations in this Inbox. |
| **Label inboxes** | Full access and ownership for every conversation in this Inbox. **Only Owners can delete individual messages from a conversation thread.** Can import and export conversations in bulk. | Can create conversations, interact with, and view conversations for this Inbox. **Can't delete messages from a conversation thread.** No bulk import or export. | Can view and interact with conversations in this inbox, but can't create new conversations or delete them. **Can't delete messages from a conversation thread.** | Blocked from viewing, editing, or deleting conversations in this Inbox. |
| **Teammate inboxes** | Full access and ownership for every conversation in this Inbox. **Only Owners can delete individual messages from a conversation thread.** Can import and export conversations in bulk. | Can create conversations, interact with, and view conversations for this Inbox. **Can't delete messages from a conversation thread.** No bulk import or export. | Can view and interact with conversations in this inbox, but can't create new conversations or delete them. **Can't delete messages from a conversation thread.** | Blocked from viewing, editing, or deleting conversations in this Inbox. |
| **Workspace profile** | Designated owner who can edit and override workspace profile information, including name, address, and company details. | Can edit workspace profile information, including name, address, and company details. | Can view workspace profile information, but can't make changes. | Blocked from workspace profile. |
| **Inbox settings** | Designated owner who can edit and override every inbox setting, including appearance, Labels, message templates, and configuration. | Can create, edit, and delete Templates and Labels for the workspace. | Can select existing Templates or Labels within the Inbox. Can't create new or edit existing Templates or Labels. | Blocked from Templates or Labels within the Inbox. No access to Inbox settings. |
| **Billing & subscriptions** | Designated owner who can fully manage billing, subscriptions, payment methods, and user seat allocations. Can upgrade, downgrade, and cancel subscriptions. | Can update billing information, manage subscriptions, and change user seats. | Can view billing information and subscription details, but can't make changes. | Blocked from billing information and subscription details. |
| **Team management** | Designated owner who can't be removed from the workspace. They can fully manage team members, invite users, create/edit roles, and reset passwords. | Can invite teammates, remove teammates, manage user roles, and perform team administration. | Can view team member information and roles, but can't make changes. | Blocked from team management with no ability to remove teammates. |
| **Phone Management** | Designated owner who can fully manage phone numbers, including adding, removing, and configuring every phone setting. | Can manage phone number settings and configuration for the workspace. | Can view phone number information, but can't modify phone settings. | Blocked from phone management settings and number information. |
| **Custom Fields** | Designated owner who can fully manage [custom fields](/settings/workspace-settings/custom-fields), including creating, editing, and deleting every custom field definition and value. | Can create, edit, and manage custom fields for the workspace. | Can view custom field values on contacts, but can't create, edit, or delete custom fields. | Blocked from custom fields. Users see a Blocked Resource placeholder. |
| **Relay Management** | Designated owner who can fully manage Relays, including creating, editing, deleting, and configuring every automation setting. | Can create, edit, and manage Relays (automations) for the workspace. | Can view Relay configurations, but can't create, edit, or delete Relays. | Blocked from the Relays module. Can't view, create, or manage Relays. |
| **Analytics** | Full access to the workspace analytics dashboard, including every metric, channel-level stat, and user-level activity data. | Can view the workspace analytics dashboard, including every metric, channel-level stat, and user-level data. | Can view the workspace analytics dashboard with read-only access to every metric. | Blocked from the analytics dashboard. |
...
<Warning>
  **Who can delete messages from a conversation thread?**
  ...
  By default, only **Admin** users have **Owner** access to every phone line connected to your MessageDesk workspace. **Managers** and **Operators** get **Editor** access to phone number inboxes by default and **can't** delete messages.

  If you don't want a user to delete messages from a conversation thread, make sure that user **doesn't have Owner permission** on the phone line. Adjust their role or use a [custom role](#custom-roles--permissions) to grant **Editor**, **Viewer**, or **No Access** on the relevant **Phone number inboxes**.
</Warning>
...
**Who can delete messages from a conversation thread?**\
Only users with **Owner** permission on a phone number's Inbox Access can delete individual messages from a conversation thread. By default, only **Admin** users have **Owner** access to every phone line. To prevent a user from deleting messages, make sure their role doesn't have **Owner** permission on that phone line. Use **Editor**, **Viewer**, or **No Access** on **Phone number inboxes** instead.

**What happens when a message is deleted?**\
When an Owner deletes a message, it's a **soft delete**. The message is removed from the front end but MessageDesk keeps the message history on the back end. The data is exportable for compliance and record-keeping.
...
**Restrict to certain labels (Support: Urgent)?**\
