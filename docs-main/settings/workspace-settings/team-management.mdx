...
<Tip>
  Trying to decide between **per-user permissions** in one workspace and a **separate workspace**? Most teams should start here with roles and permissions. Only spin up a new workspace when you have distinct teams with **separate contact lists** that need to be managed independently. See [Workspace vs. UAC](/get-started/tenant-management#do-i-need-a-separate-workspace-or-just-user-permissions) for the full decision guide.
</Tip>
