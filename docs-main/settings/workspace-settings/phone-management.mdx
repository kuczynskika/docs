...
<Warning>
  **Conversation history stays with the source workspace.** To move a number, MessageDesk first removes it from the source workspace. This also removes every **conversation tied to that number** from that workspace. After the move, the destination workspace starts fresh, and the previous conversation history is **no longer viewable** from either workspace.
</Warning>
