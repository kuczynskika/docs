...
<Note>
  Call forwarding is currently available on **Bandwidth** and **Telgorithm** phone lines. **Twilio** call forwarding is temporarily disabled while additional configuration is finalized and will be enabled in an upcoming release.
</Note>

### Call logs

Each phone number's **Phone Profile** page includes a **Call Logs** table showing every inbound call that was forwarded, along with the forwarding details for that call.
...
When an inbound call is forwarded, a **Call Forwarded** event is added to the matching conversation thread in the Inbox, so your team has full context on voice activity alongside the SMS history.
