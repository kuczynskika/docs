...
<Note>
  Call forwarding works on **Bandwidth**, **Telgorithm**, and **Twilio** phone lines. **Twilio BYOC** numbers can't configure call forwarding in-app today—the setup process differs significantly. If you need forwarding on a BYOC number, contact support and we can configure it manually on a per-number basis.
</Note>
