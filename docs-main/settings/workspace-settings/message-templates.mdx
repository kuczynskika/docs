...
<Note>
  **Media-only templates.** In the templates list, templates that contain only media (no text body) display **"Contains MMS"** in the **text** column instead of appearing blank, so you can spot them at a glance.
</Note>
...
You can move templates in and out of MessageDesk in bulk from the [Data Center](/settings/workspace-settings/data-center).

- **Import templates.** Upload a CSV of templates and map columns to name, body, tags, and media. The importer updates any template that matches an existing template by **name** in place. See [Import message templates](/settings/workspace-settings/data-center#import-message-templates).
- **Export templates.** Run a template export from the Data Center to get a CSV of every template in the workspace for backup or migration.
