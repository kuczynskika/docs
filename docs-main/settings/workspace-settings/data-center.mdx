...
Templates that match an existing template by **name** are updated in place. New names create new templates. See [Message Templates](/settings/workspace-settings/message-templates) for how the library is managed after import.
...
Labels that match an existing label by **name** are updated with the imported color and icon. See [Labels](/settings/workspace-settings/labels) for how the shared label library works.

### Import opt-outs

Move an existing opt-out list into MessageDesk so those numbers are blocked from all outbound sends in the workspace.
...
<Warning>
  Opt-out imports are **workspace-scoped**. Each imported number is blocked from every phone line in this workspace. If your tenant has multiple workspaces, run the import in each one that needs the block.
</Warning>

Phone numbers already on the workspace opt-out list are skipped as duplicates. See [Anonymous, opted-out, and blocked contacts](/contacts/anonymous-opt-out-blocked) for what opting out means for a contact.
