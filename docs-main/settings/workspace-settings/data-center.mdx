---
title: "Import, export, and search message history"
...
og:title: "Data Center: import and message history"
...
<Tip>
  Looking to **view or export existing messages**? Jump to [Message History](#message-history). To **export contacts**, jump to [Export](#export). To export a **single conversation** directly from the Inbox toolbar, see [Export a conversation](/inbox/export-conversation).
</Tip>

<Note>
  Message History exports, contact exports, and per-conversation exports all run through the same **universal export system**. Each one processes asynchronously in the background. MessageDesk delivers the file through the **Export Available** button in the top nav, plus an **export completed email** to the user who triggered it.

  Imports work the same way. When a contact, message, message template, label, or opt-out import finishes, MessageDesk sends an **import completed email** to the user who started it with a summary of the run.
</Note>
...
![Data Import](/images/data-import.png "Data Import")
...
The importer updates templates that match an existing template by **name** in place. New names create new templates. See [Message Templates](/settings/workspace-settings/message-templates) for how the library is managed after import.
...
The importer updates any label that matches an existing label by **name** with the imported color and icon. See [Labels](/settings/workspace-settings/labels) for how the shared label library works.
...
MessageDesk skips phone numbers already on the workspace opt-out list as duplicates. See [Anonymous, opted-out, and blocked contacts](/contacts/anonymous-opt-out-blocked) for what opting out means for a contact.
...
- **CSV only** (`.csv`).
- **UTF-8** encoding preferred. MessageDesk strips a byte order mark (BOM) and other invalid bytes automatically.
- Include a **header row**.
- Each row must have the **same number of columns** as the header.

**Required data**

- For **contacts**: **phone**. Email and name columns are optional
...
<Frame>
  ![Data Center Export By Phone Number](/images/data-center-export-by-phone-number.png)
</Frame>
...
<Note>
  ...
  See [Notification Preferences](/settings/my-settings/notification-preferences) to manage push and browser notifications for the same event.
</Note>
...
- For **non-renderable media** (videos and other unsupported file types), the PDF shows a direct **S3 link to the file** alongside the note **"Could not load preview."** The attachment stays accessible through the link.
...
- **Custom properties.** Each [custom field](/settings/workspace-settings/custom-fields) on the workspace becomes its own column. Contacts without a value for a given custom property leave that cell blank.
...
Notifications only go to the **user who triggered the export**, not to your whole team. See [Notification Preferences](/settings/my-settings/notification-preferences) to manage push and browser notifications for the same event.
...
**What's the difference between Inbox search and Message History?**<br />The [Inbox](/inbox/filter-search-sort) is for working live conversations. **Message History** is the workspace-wide archive built for audit, reporting, and bulk export.

**What format are exports in?**<br />**CSV** for data and reporting, or **PDF** for shareable conversation records.

**Can I export Message History across every phone line at once?**<br />Yes. In **Settings → Data Center → Message History**, leave every line checked in the **Phone Numbers** filter and click **Export**. That produces a single file covering the entire workspace. To export just one line, uncheck the others before exporting.

**Can I export Message History across all workspaces on my tenant?**<br />No. Message History exports are scoped to a **single workspace**. If your tenant has multiple workspaces, run the export in each workspace separately.

**Can I export my contacts?**<br />Yes. Open **Settings → Data Center → [Export](#export)** and click **Export Contacts** to run a full CSV dump of every contact in the workspace. Filtered contact exports (by label, custom field, saved filter, and so on) aren't supported yet.

**How do I know when an export is ready?**<br />Every export delivers two ways: an **Export Available** button in the top nav, and an **export completed email** with a download link to the user who triggered it. Contact, Message History, and per-conversation exports all share the same in-app notification because they all run through the universal export system.

**Are media attachments included in exports?**<br />
