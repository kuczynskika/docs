...
- **Invalid data** (bad phone/email format) is reported so you can fix and re-upload.
- Problem rows are **skipped** during processing.

### Monitor progress

After you start the import, you'll see the real-time status. When the import finishes, a summary shows total **processed**, **imported**, **skipped**, and any **error reasons**.

<Note>
  Need a walkthrough? See [Create & Import Contacts](/contacts/create-import) for step-by-step guidance.
</Note>
...
- **Channel requirement.** Messages can only import into **outbound channels that already exist** in your workspace.
...
MessageDesk checks **email** and **phone**. When a row matches an existing contact, that contact is **updated** with the values from the import. Updates only apply to columns you mapped, and only when the mapped column has a value. Empty cells leave existing fields unchanged. Updates apply to standard fields (name, email, phone, address) and custom fields.
...
- **Status.** Delivery state (delivered, failed).
