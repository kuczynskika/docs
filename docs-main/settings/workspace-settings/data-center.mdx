...
The **Export** tab runs a full CSV export of every contact in your workspace. Use it when you need a complete backup of your contact list for archiving, reporting, or moving data to another system.
...
<Note>
  **Contact exports are a full workspace dump.** Every contact in the workspace is included. Filtered contact exports (by label, custom field, saved filter, and so on) aren't supported in this version.
</Note>
...
Each row in the CSV contains a contact's standard fields (name, phone numbers, email, address, and so on) plus any [custom fields](/settings/workspace-settings/custom-fields) defined in your workspace.
...
**How do I know when an export is ready?**<br />Every export delivers two ways: an **Export Available** button in the top nav, and an **export completed email** with a download link to the user who triggered it. The same in-app notification is used across contact, Message History, and per-conversation exports because they all run through the universal export system.
