...
<Note>
  **Required for the Send Webhook relay action.** This configuration must exist before the [Send Webhook](/relays/webhooks#outbound-webhooks) action will deliver events from your Relays.
</Note>
...
For the full webhook payload format, headers, signature validation algorithm, and retry behavior, see [Relays → Send Webhook](/relays/webhooks#outbound-webhooks).
...
- [Relays → Send Webhook](/relays/webhooks#outbound-webhooks). Payload, signature validation, and retry behavior.
