...
- **Export Available.** Push and browser notification to the user who triggered any [conversation export](/inbox/export-conversation), [Message History export, or contact export](/settings/workspace-settings/data-center) when the file is ready to download from the **Export Available** button in the top nav. MessageDesk also sends an **export completed email** with a download link to the same user for every export type.
