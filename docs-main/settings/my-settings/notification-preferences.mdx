...
- **Export Available.** Push and browser notification to the user who triggered any [conversation export](/inbox/export-conversation), [Message History export, or contact, message template, label, or opt-out export](/settings/workspace-settings/data-center). The file is ready to download from the **Export Available** button in the top nav. MessageDesk also sends an **export completed email** with a download link to the same user for every export type.
- **Import completed email.** When a [Data Center import](/settings/workspace-settings/data-center) (contacts, messages, message templates, labels, or opt-outs) finishes, MessageDesk emails the user who started it a summary of the run. Import notifications only go by email; there's no push or browser toggle for this event.
...
- The badge shows the **total** unread conversations you have access to in the workspace. It **isn't** filtered by your Notification Preference setting. For example, if you're set to "Only notify me about conversations assigned to me," the badge still shows the full unread conversation count.
- The badge counts **unread conversations**, not unread messages. Receiving multiple messages from the same contact in an already-unread conversation doesn't increase the badge.
