...
| **Custom Fields**           | Create, edit, and manage [custom fields](/settings/workspace/custom-fields) on contacts        | Owner     | Editor      | Viewer       |
...
<Warning>
  Plan your roles carefully before creating them — custom roles cannot be deleted at this time. You can always rename or edit a role later.
</Warning>
...
### How to edit or rename a custom role

1. **Account → Settings → Team Management**
2. Click the **custom role** you want to edit.
3. Update the **role name** or adjust **permissions** as needed.
4. **Save changes** — updates apply immediately (users may need to refresh).

<Note>
  Custom roles cannot be deleted at this time. If you created a role by mistake, rename it (e.g., "Do Not Use") and stop assigning it to teammates.
</Note>
...
Not yet—role deletion isn't available from the UI. As a workaround, you can **rename** the unwanted role (e.g., "Do Not Use") and stop assigning it to teammates. Role deletion is planned for a future update.

**Can Managers delete things?**\
No. **The editor** cannot delete. Only the **Owner** can delete.

**What happens when a message is deleted?**\
When a user deletes a message, it's a **soft delete**—the message is removed from the front end but MessageDesk retains the message history on the back end. This data is exportable for compliance and record-keeping purposes.
