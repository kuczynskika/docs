---
title: "Team management: invites, roles, and permissions"
description: "Invite teammates, manage Admin, Manager, Operator, and custom roles, set per-feature permissions, and control inbox access for every user in MessageDesk."
...
<Warning>
  **Who can delete messages from a conversation thread?**
  ...
  Only users with **Owner** permission on a phone number's Inbox Access can delete individual messages from a conversation thread. **Editors**, **Viewers**, and users with **No Access** cannot delete messages.

  By default, only **Admin** users have **Owner** access to all phone lines connected to your MessageDesk workspace. **Managers** and **Operators** receive **Editor** access to phone number inboxes by default and **cannot** delete messages.

  If you do not want a user to be able to delete messages from a conversation thread, make sure that user **does not have Owner permission** on the phone line. Adjust their role or use a [custom role](#custom-roles--permissions) to grant **Editor**, **Viewer**, or **No Access** on the relevant **Phone number inboxes**.
</Warning>
