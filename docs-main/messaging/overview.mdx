...
MessageDesk sends and receives over standard carrier SMS and MMS. It does not use Apple iMessage.

- iMessage is Apple's separate, proprietary protocol. iMessage conversations do not appear in the MessageDesk shared Inbox.
- Messages from iPhone users still reach you. When you or a contact texts a MessageDesk number, the message is delivered as SMS/MMS and shows up in the Inbox normally. You just won't see blue-bubble iMessage threads.
- An existing cellular or mobile line can't be connected to MessageDesk while it stays with its wireless carrier. If a team wants to share texting on a personal cell, the practical option is a separate MessageDesk business number, with [call forwarding](/settings/workspace-settings/phone-management#call-forwarding) to a cell if voice matters. See [Phone numbers](/get-started/phone-numbers/overview) to add or connect a line.
