...
- **Outbound:** `10:42 AM | Delivered ✓` (timestamp **|** status + icon). Same-day messages show a relative time instead (for example, `5 minutes ago | Delivered ✓`); once the message is from a previous day, the actual time replaces it.
- **Failed:** Red/error styling; click to open details and **Resend**
- **Media:** Status shows even without text content
- **Inbound:** Timestamp only (no status). Same-day messages show a relative time (for example, "2 hours ago"); messages from a previous day show the actual time.
