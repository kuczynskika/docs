...
Get messages through, fast. Here's what each status means, how to fix failures, and what to send support if you're stuck.
...
- **Outbound:** `10:42 AM | Delivered ✓` (timestamp **|** status + icon). Same-day messages show a relative time instead (for example, `5 minutes ago | Delivered ✓`). Messages from a previous day show the actual send time.
- **Failed:** Red/error styling; click to open details and **Resend**
- **Media:** Status shows even without text content
- **Inbound:** Timestamp only (no status). Same-day messages show a relative time (for example, "2 hours ago"). Messages from a previous day show the actual send time.
