...
## How to Attach Media to a Message _or_ a Comment on Desktop
...
### 2. Select an Image/Video from your phone's camera roll:
...
<Warning>
  **Known Limitations: iOS voice messages (.amr files)** or voice messages sent from iOS devices use the `.amr` format, which is not natively supported by most browsers. These files cannot currently be played back in MessageDesk.
</Warning>
