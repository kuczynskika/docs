---
title: "Send and Receive MMS Media Files"
description: "Send and receive images, videos, PDFs, and documents through MessageDesk MMS. Supported file types, size limits, and best practices for delivery success."
sidebarTitle: "Media"
...
## Upload limits and delivery behavior
...
- **Delivery behavior:** Images over 500 KB are **compressed first**; if compression can't get them under the threshold, they send as **clickable links**
...
### How compression vs. link conversion works

When an image is **larger than 500 KB**, MessageDesk tries to compress it automatically so it can meet carrier requirements.

- **Under ~500 KB to start:** sends as-is, no compression.
- **Over ~500 KB:** MessageDesk attempts to resize and compress the image below the threshold. Heavy compression on very large source files (or formats like **.png**) can sometimes make the image appear blurry or distorted.
- **Compression can't get it under the threshold:** MessageDesk converts the file into a **clickable link** instead so it still delivers reliably. The linked version preserves the original quality.

<Note>
  There's no exact "above X KB and it always becomes a link" cutoff. Whether a file compresses successfully or converts to a link depends on the image type, dimensions, and how much it can be reduced. As a rule of thumb, **anything above 500 KB will either be compressed or sent as a link if compression fails.**
</Note>

<Tip>
  **Recommended image size: ≤ 500 KB.** Starting from a file already under 500 KB skips compression entirely and preserves the sharpest image. That matters for branded product photos. For larger files, **.jpg** compresses much more cleanly than **.png**.
</Tip>

---

## How to attach media to a message or a comment on desktop
...
1. In the message compose box, click the **\+** icon.
   ...
   ![Media Add Pn](/images/media-add.png)
2. Choose one:
...
<img
  src="/images/mobile-media-upload.png"
  alt="Mobile Media Upload"
  title="Mobile Media Upload"
  className="mx-auto"
  style={{ width:"61%" }}
/>
...
MessageDesk first tries to **compress** the image below 500 KB so it sends as an inline MMS. If compression can't get it under the threshold, it sends as a **clickable link** so your recipient can view or download it reliably. There's no exact size at which a file always becomes a link. It depends on the image type and how much it can be reduced.

**Why does my image look smaller, blurry, or distorted?**\
When an image is larger than 500 KB, MessageDesk tries to compress it automatically so it can meet carrier requirements. That compression can sometimes make the image appear blurry or distorted, especially if the original file is very large or in a format like **.png**. If MessageDesk can't compress the file enough, it converts the file into a **link** instead to improve deliverability. To keep your branding sharp, **export images at ≤ 500 KB before attaching** so compression is skipped entirely.
